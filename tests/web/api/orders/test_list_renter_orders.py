...
async def test_get_list_renter_orders(mock_cache: AsyncMock, auth_client: AsyncClient) -> None:
...
@patch('src.services.cars_cache.CarCacheService.get_car', new_callable=AsyncMock)
async def test_get_list_renter_with_filters(mock_cache: AsyncMock, filter, count, auth_client: AsyncClient) -> None:
    orders = await OrderFactory.create_batch(3, renter_id=auth_client.user_id)
    mock_cache.return_value = {'id': "bimbimbambam", 'model': 'BMW'}
...
@patch('src.services.cars_cache.CarCacheService.get_car', new_callable=AsyncMock)
async def test_get_list_renter_pagination(mock_cache: AsyncMock, auth_client: AsyncClient) -> None:
